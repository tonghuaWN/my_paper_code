...
def numerator(X: np.ndarray, cls_index: np.ndarray, cls_n_ex: np.ndarray, f: int) -> float:
    ...
    for j in range(len(cls_n_ex)):
        for k in range(len(cls_n_ex)):
            summation += cls_n_ex[j] / sum(cls_n_ex) * cls_n_ex[k] / sum(cls_n_ex) * np.power(
                (np.mean(X[cls_index[j], f]) - np.mean(X[cls_index[k], f])), 2)
    return summation
...
def denominator(X: np.ndarray, cls_index: np.ndarray, cls_n_ex: np.ndarray, f: int) -> float:
    summation = 0
    for j in range(len(cls_n_ex)):  # 循环三次
...
def compute_rfi(X: np.ndarray, cls_index, cls_n_ex) -> float:
    ls = []
    for i in range(np.shape(X)[1]):  # 4次循环
...
def numerator(X: np.ndarray, cls_index, cls_n_ex, i) -> float:
    numerator_list = [cls_n_ex[j] * np.power((np.mean(X[cls_index[j], i]) - np.mean(X[:, i], axis=0)), 2) for j in
                      range(len(cls_index))]
    return np.sum([cls_n_ex[j] * np.power((np.mean(X[cls_index[j], i]) - np.mean(X[:, i], axis=0)), 2) for j in
                   range(len(cls_index))])


# 对论文公式的改造版本

def numerator_paper(X: np.ndarray, cls_index, cls_n_ex, i, j) -> float:
    numerator = cls_n_ex[j] * np.power((np.mean(X[cls_index[j], i]) - np.mean(X[:, i], axis=0)), 2)
    return numerator


# according to aquation(3)

def denominator(X: np.ndarray, cls_index, cls_n_ex, i) -> float:  # X:[150,4] i:0-4 列数　　cls_index[j]:[0:50]
    return np.sum([np.sum(np.power(X[cls_index[j], i] - np.mean(X[cls_index[j], i], axis=0), 2)) for j in
                   range(0, len(cls_n_ex))])
...
def compute_rfi_paper(X: np.ndarray, cls_index, cls_n_ex, j) -> float:
    ls = []
    for i in range(np.shape(X)[1]):
        if denominator(X, cls_index, cls_n_ex, i) != 0:

            ls.append(numerator_paper(X, cls_index, cls_n_ex, i, j) / denominator(X, cls_index, cls_n_ex, i))
    return ls


def ft_F1_paper(X: np.ndarray, cls_index: np.ndarray, cls_n_ex: np.ndarray, j) -> float:
    return 1 / (1 + np.max(compute_rfi_paper(X, cls_index, cls_n_ex, j)))


def ft_F1(X: np.ndarray, cls_index: np.ndarray, cls_n_ex: np.ndarray) -> float:
    return 1 / (1 + abs(np.max(compute_rfi(X, cls_index, cls_n_ex))))
...
def dVector(X: np.ndarray, y_class1: np.ndarray, y_class2: np.ndarray) -> float:
    X_class1 = X[y_class1]
    u_class1 = np.mean(X_class1, axis=0)
    X_class2 = X[y_class2]
    u_class2 = np.mean(X_class2, axis=0)
...
def ft_F4(X: np.ndarray, ovo_comb: np.ndarray, cls_index: np.ndarray, cls_n_ex: np.ndarray) -> np.ndarray:
    ...
    for idx1, idx2 in ovo_comb:
        ...
        while X_.shape[1] > 0 and X_.shape[0] > 0:
            ...
            idx_min, _, overlapped_region_by_feature = _compute_f3(X_, _minmax(X_, y_class1, y_class2),
                                                                   _maxmin(X_, y_class1, y_class2))
