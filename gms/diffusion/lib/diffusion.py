...
from util.utils import get_variance
...
class GaussianDiffusion(nn.Module):
    ...
    def q_sample(self, x_0, t, noise=None):
        ...
        sqrt_var = extract(self.sqrt_one_minus_alphas_cumprod, t, x_0.shape)
        return (extract(self.sqrt_alphas_cumprod, t, x_0.shape) * x_0
                + sqrt_var * noise), sqrt_var
    ...
    @torch.no_grad()
    def p_sample_loop(self, model, shape, noise_fn=torch.randn, end=None, x_1=None):
        ...
        for i in reversed(range(end, 1000)):
            img = self.p_sample(
                model,
                img,
    ...
    def training_losses(self, model, x_0, t, noise=None):
        ...
        x_t, sqrt_var = self.q_sample(x_0=x_0, t=t, noise=noise)
        if self.loss_type == 'kl':
            ...
            a = 0.8
            l2_term = torch.square(target - model_output)
            params = a * sqrt_var * x_t + (1 - a) * model_output
            losses = torch.mean((target - params).view(x_0.shape[0], -1) ** 2, dim=1)
