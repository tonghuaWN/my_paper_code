...
class DiffusionModel(utils.GM):
    ...
    def evaluate(self, writer, x, epoch):
        ...
        gs = th.stack(samples)
        gp = th.stack(preds)
        writer.add_video('sampling_process', utils.combine_imgs(gs.permute(1, 0, 2, 3, 4), 5, 5)[None, :, None], epoch,
                         fps=60)
        # at any point, we can guess at the true value of x_0 based on our current noise
        # this produces a nice visualization
        writer.add_video('pred_xstart', utils.combine_imgs(gp.permute(1, 0, 2, 3, 4), 5, 5)[None, :, None], epoch,
                         fps=60)
