...
class PixelCNN(utils.Autoreg):
    ...
    def __init__(self, C):
        ...
        if C.use_resblock:
            ...
            def block_init():
                return MaskConv2d('B', C.n_filters, C.n_filters, kernel_size=C.kernel_size, padding=C.kernel_size // 2)

        model = nn.ModuleList(
            [MaskConv2d('A', n_channels, C.n_filters, kernel_size=C.kernel_size, padding=C.kernel_size // 2)])
