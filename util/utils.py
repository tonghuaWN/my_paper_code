...
from complexity_measurement.measures import compute_F1
import random
...
def init_processes(rank, size, fn, args):
    ...
    os.environ['MASTER_PORT'] = '6021'  # 6020-->6021
...
def common_x_operations(x, num_x_bits):
    ...
    # change bit length
    x = change_bit_length(x, num_x_bits)  # 对x没有任何操作
    x = symmetrize_image_data(x)  # 2.0×x-1.0

    return x


def common_x_operations_paper(x, num_x_bits):
    ...
    # change bit length
    x1 = change_bit_length(x1, num_x_bits)  # 对x没有任何操作
    x1 = symmetrize_image_data(x1)  # 2.0×x-1.0

    return x1, label


def generate_tau_0(tau_0, label):
    result = []
    numpy_tau_0 = tau_0.cpu().detach().numpy()
    numpy_label = label.cpu().detach().numpy()
    tau_0_new = compute_F1(numpy_tau_0, numpy_label)
    for i in range(label.size()[0]):
        result.append(tau_0_new[label[i]])
    tensor_tau_0 = torch.Tensor(result).cuda()
    return 1.0-tensor_tau_0


def generate_tau_0_(tau_0, label):
    numpy_tau_0 = tau_0.cpu().detach().numpy()
    numpy_label = label.cpu().detach().numpy()
    tau_0_new = compute_F1(numpy_tau_0, numpy_label)
    tau_0_new_ = []
    for i in tau_0_new:
        arr = 1 - i
        tau_0_new_.append(arr)
    l2 = sorted(tau_0_new_)
    tau_list = []
    result_list = []
    for i in l2:
        index = tau_0_new_.index(i)
        tau_list.append(index)  # 复杂度的相对顺序关系，从左到右依次简单，难-->简
    # for i in numpy_label.tolist():
    #     range_tau = tau_list.index(i)
    #     tau_item = random.uniform(range_tau * 0.1, range_tau * 0.1 + 0.1)
    #     result_list.append(tau_item)
    # tensor_tau_0 = torch.Tensor(result_list).cuda()
    # return tensor_tau_0
    return tau_list

def calculate_kl_(mean, log_var):
    kl = -0.5 * torch.sum(1 + log_var - mean ** 2 - log_var.exp(), dim=[1, 2, 3])
    return kl